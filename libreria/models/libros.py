...
class LibroManager(models.Manager):
    ...
    def libros_con_mas_de_200_paginas(self):
        consulta = self.filter(paginas__gt=200).exclude(
            isbn__in=('1933988592', '1884777600')).order_by('paginas')
        return consulta

    def libros_con_mas_o_igual_a_200_paginas(self):
        consulta = self.filter(paginas__gte=200).filter(
            estatus='P').values('isbn', 'paginas').order_by('paginas')
        return consulta
    ...
    def numero_de_libros_de_python_agrupados_por_categoria(self):
        consulta = self.filter(
            categoria__contains='python').aggregate(Sum('paginas'))
        return consulta

    # Agrupaciones

    def numero_de_libros_de_python_agrupados_por_categoria(self):
        consulta = self.filter(categoria__contains='python').values('categoria').annotate(
            NumeroLibros=Count('*'), PromedioPaginas=Avg('paginas'), MinPaginas=Min('paginas'))
        return consulta

    def numero_de_libros_de_python_agrupados_por_categoria_y_editorial(self):
        consulta = self.filter(categoria__contains='python').values(
            'categoria', 'editorial__nombre').annotate(NumeroLibros=Count('*'))
        # return consulta
        return consulta.filter(NumeroLibros__gt=1)

    def fechas_de_publicacion_con_mas_de_5_libros(self):
        consulta = self.values('fecha_publicacion').annotate(
            cant_fec_pub=Count('fecha_publicacion')).filter(cant_fec_pub__gte=5)
        return consulta
    ...
    def desc_corta_resumida(self):
        consulta = self.annotate(longitud=Length('desc_corta')).annotate(
            desc_resumida=Case(
                When(longitud__gt=50, then=Concat(
                    Left('desc_corta', 50), V('...'))),
                default=('desc_corta'),
    ...
    def titulo_inicia_igual_que_su_descripcion_corta(self):
        consulta = self.annotate(tit50=Left('titulo', 50), desc50=Left(
            'desc_corta', 50)).filter(tit50=F('desc50')).values('isbn', 'tit50', 'desc50')
        return consulta

    def mostrar_categoria_sin_comilla(self):
        consulta = self.annotate(categoria_sin_comillas=Replace('categoria', V('"'), V(''))).values(
            'isbn', 'categoria', 'categoria_sin_comillas').filter(categoria__contains='python')
        return consulta

    def modfica_columna_categoria_quitando_corchetes(self):
        self.filter(categoria='[]').update(
            categoria=Replace('categoria', V('[]'), V('')))

    def libros_de_categoria_python_O_Java_O_net_version_corta(self):
        consulta = self.filter(Q(categoria__contains='python') | Q(
            categoria__contains='java') | Q(categoria__contains='net') & ~Q(paginas=0))
        return consulta

    def consulta_libros_con_cronica(self):
        consulta = self.select_related('librocronica').filter(
            categoria__contains='python')
        return consulta
    ...
    def consulta_libros_editorial_muchas_consultas_forma2(self):
        consulta = self.all().select_related(
            'editorial').filter(categoria__contains='python')
        for libro in consulta:
            print(libro.editorial.nombre)
        return consulta
...
# prefetch_related
    ...
    def autores_con_sus_libros_escritos_forma_optima(self):
        ...
        autores = Autor.objects.filter(
            pk__in=(398, 523)).prefetch_related('libro')
        for autor in autores:
            print(f'Autor: {autor}')
            print('Libros escritos:')
            for libro in autor.libro.all():
    ...
    def autores_con_sus_libros_escritos_y_editoriales_forma_optima(self):
        from .autores import Autor
        autores = Autor.objects.filter(
            pk__in=(398, 523)).prefetch_related('libro__editorial')
        for autor in autores:
            print(f'Autor: {autor}')
            print('Libros escritos:')
            for libro in autor.libro.all():
    ...
    def autores_con_sus_libros_escritos_y_editoriales_usando_prefecth_con_atributo(self):
        ...
        libro_y_editorial = Libro.objects.filter(
            titulo__contains='u').select_related('editorial')
   ...
   # Relacion inversa Libros - Autores
    def libros_con_sus_autores_y_su_editorial_optima(self):
        libros = self.filter(isbn__in=('1617290475', '1935182048')).select_related(
            'editorial').prefetch_related('libros_autores')

        for p in libros:
            print(
                f'{p.isbn} - {p.titulo} Editorial: {p.editorial.nombre}  Escrito por:')
...
class Libro(models.Model):
    ...
    titulo = models.CharField(
        max_length=70, blank=True, validators=[validar_titulo, ])
